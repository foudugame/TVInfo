...
window.addEventListener("load", function() {	
    $(".cmdSendAction").click(function(event){
        ...
        modalConfim(data_title, data_message, data_alert, data_textY, data_textN,function (){
		    ...
		    this_load.hide();
		    this_error.hide();
		    $.post({ url: "http://192.168.0.241/" + data_action, timeout:500 }, { cmd: data_cmd, token: TOKEN }).done(function( ers  ) {	
			    if (ers.status) {
		            this_success.show();
		            this_load.hide();
		            this_error.hide();
...
function infostats() {
    ...
    $.post({ url: "http://192.168.0.241/stats", timeout:500 }, { opt: optget, token: TOKEN }).done(function( cfg ) {
	    //console.log(cfg);
	    var config = cfg.setting;//JSON.parse();
	    //var config = cfg.wifiEtats;
	    //console.log(cfg);
		//$("#debug").val(false).change();
...
56	
					*/
			        ...
			        if (checkExists(".textWidth_flash_used_percent")) {//d+"%"
			        ...
			        if (checkExists(".textWidth_getRamUsage_percent")) {//d+"%"
...
function formSend(arr) {
    arr["token"] = TOKEN;
    $.post({ url: "http://192.168.0.241/save", timeout:500 } , arr).done(function( res ) {			   
		var colorBG, colorTITLE, colorBTN1, colorBTN2;
...
function removeNotifications(id, event) {
	if (checkExists("#notifications-id-" + id)) {
		$.post({ url: "http://192.168.0.241/notifications", timeout:500 }, { erase: id, token: TOKEN }).done(function( ers  ) {	
			if (ers.status) {
		        $("#notifications-id-" + id).remove();
			}
		});
...
function updateFile(_this) {
  ...
  $('.upload_name').text("Fichier: " + file.name);	
  ...
  ajax.open("POST", "http://192.168.0.241/upload", true);
  ajax.send(formdata);
}

function progressHandler(event) {
  var percent = (event.loaded / event.total) * 100;
  $('.upload_info').text("Uploaded " + event.loaded + " bytes | " + Math.round(percent) + "%");
  if (percent >= 100) {
	$(".textWidth_getRamUsage_percent").width(Math.round(percent) + "%");
  }
}
